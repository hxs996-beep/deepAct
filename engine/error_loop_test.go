...
func TestErrorLoopState_SuccessResetsStreak(t *testing.T) {
	...
	if a := s.Check(key, false); a.Type != GuardBlock {
...
func TestErrorLoopState_NilSafe(t *testing.T) {
	var s *LoopTracker
	if a := s.Check("edit:x", false); a.Type != GuardAllow {
