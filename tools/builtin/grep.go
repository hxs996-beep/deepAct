...
func (t *GrepTool) Spec() tools.ToolSpec {
	return tools.ToolSpec{
		Name:        "grep",
		Description: "Search file contents for a pattern. Supports regex (default) or exact substring matching. Can include context lines around each match for better understanding. Primary tool for cross-file exploration: finding all occurrences of a pattern/error string, all call sites of a function, or tracing a flow — grep first rather than reading files one by one. For finding symbol definitions, type info, or usages, prefer `lsp workspaceSymbol`/`lsp hover`/`lsp findReferences` instead — they are more precise and cheaper.",
